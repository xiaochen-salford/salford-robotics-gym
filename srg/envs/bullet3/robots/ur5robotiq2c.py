...
class RobotUR5RobotiqC2(RobotEnv):
    ...
    def __init__(self, initial_pos=[0,0,0], 
                 inital_orn=bullet.getQuaternionFromEuler([0,0,0])):
        ...
        # self.joints_num = self.config.joints_num
        ...
        # self.arm_joint_idx_range = range(0, self.arm_dof_num)
        # self.griper_idx_range = range(self.arm_dof_num, self.arm_link_num+self.gripper_dof_num)
        # self.griper_joint_idx_range = range(self.arm_dof_num, self.arm_link_num+self.gripper_dof_num)

        # self.joint_indices = range(self.arm_dof_num+self.gripper_dof_num)
        
        super().__init__(self.config, initial_pos, inital_orn)
    ...
    def get_state(self, form='flat'):
        if form == 'dict_lik': 
            ...
            end_effector_state = {}
            end_effector_state['link_world_pos'] = np.array(ee_state[0])
            end_effector_state['link_world_orn'] = np.array(ee_state[1])
