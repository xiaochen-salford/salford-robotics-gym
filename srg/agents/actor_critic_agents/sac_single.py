...
class SACSingle(BaseAgentSingle):
    """
    ...
    def __init__(self, config):
        ...
        self.critic_local = self.create_NN(input_dim=self.state_size + self.action_size, 
                                           output_dim=1, 
                                           key_to_use="Critic" )
        self.critic_local_2 = self.create_NN(input_dim=self.state_size + self.action_size, 
                                             output_dim=1,
                                             key_to_use="Critic", 
        ...
        self.critic_target = self.create_NN(input_dim=self.state_size + self.action_size, 
                                            output_dim=1,
                                            key_to_use="Critic" )
        self.critic_target_2 = self.create_NN(input_dim=self.state_size + self.action_size, 
                                              output_dim=1,
                                              key_to_use="Critic" )
        BaseAgentSingle.copy_model_over(self.critic_local, self.critic_target)
        BaseAgentSingle.copy_model_over(self.critic_local_2, self.critic_target_2)
        ...
        if self.automatic_entropy_tuning:
            ...
            self.target_entropy = -torch.prod(torch.tensor(self.environment.get_action_size())) # heuristic value from the paper
    ...
    def reset_game(self):
        ...
        BaseAgentSingle.reset_game(self)
    ...
    def produce_action_and_action_info(self, state):
        ...
        actor_output = self.actor_local(state).clone()
        mean, log_std = actor_output[:, :self.action_size], actor_output[:, self.action_size:]
        mean = torch.clamp(mean, min=-1000, max=1000)
        log_std = torch.clamp(log_std, min=0.1, max=3)
        std = log_std.exp()
        normal = Normal(mean, std)
    
        # action = normal.rsample()  #rsample means it is sampled using reparameterisation trick
        action = normal.rsample()  #rsample means it is sampled using reparameterisation trick
    ...
    def update_all_parameters(self, critic_loss_1, critic_loss_2, actor_loss, alpha_loss):
        ...
        self.take_optimisation_step(self.critic_optimizer, 
                                    self.critic_local, critic_loss_1,
                                    self.hyperparameters["Critic"]["gradient_clipping_norm"] )
        self.take_optimisation_step(self.critic_optimizer_2, 
                                    self.critic_local_2, critic_loss_2,
                                    self.hyperparameters["Critic"]["gradient_clipping_norm"] )
        self.take_optimisation_step(self.actor_optimizer, 
                                    self.actor_local, actor_loss,
