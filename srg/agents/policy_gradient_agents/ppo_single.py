...
from srg.agents.base_agent import BaseAgent
from srg.agents.exploration_strategies.epsilon_greedy_exploration import Epsilon_Greedy_Exploration
from srg.agents.utilities.Single_Experience_Generator import Single_Experience_Generator
from srg.agents.utilities.Utility_Functions import normalise_rewards, create_actor_distribution

class PPOSingle(BaseAgentSingle):
    """Proximal Policy Optimization agent"""

    agent_name = "PPOSingle"

    def __init__(self, config):
        BaseAgentSingle.__init__(self, config)
        self.policy_output_size = self.calculate_policy_output_size()
        if hasattr(config, 'load_previous_model') and config.load_previous_model == True:
            self.policy_new = torch.load(config.file_to_save_learnt_model)
            self.policy_old = torch.load(config.file_to_save_learnt_model)
        ...
        self.experience_generator = Single_Experience_Generator(self.environment, self.policy_new, self.config.seed,
    ...
    def calculate_log_probability_of_actions(self, policy, states, actions):
        ...
        # FIX IT: do i need the transpose below?
        policy_distribution_log_prob = policy_distribution.log_prob(actions.transpose(0,1)).transpose(0,1)
    ...
    def take_policy_new_optimisation_step(self, loss):
        ...
        torch.nn.utils.clip_grad_norm_(self.policy_new.parameters(), 
                                       self.hyperparameters[ "gradient_clipping_norm"]) # clip gradients to help stabilise training
    ...
    def locally_save_policy(self):
        torch.save(self.policy_new, self.config.file_to_save_learnt_model)